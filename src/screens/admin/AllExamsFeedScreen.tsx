...
import { StudyMaterial, studyMaterialApi } from '../../api/studyMaterialApi';

const AllMaterialsFeedScreen = ({ route, navigation }: { route: any; navigation: any }) => {
  const { theme } = useTheme();
  
  const [materialsList, setMaterialsList] = useState<StudyMaterial[]>(route.params?.materialsList || []);
  ...
  const handleDeleteMaterial = (id: string) => {
    Alert.alert(
      'Confirm Deletion',
      'Are you sure you want to softly delete this material?',
      [
          ...
          onPress: async () => {
            try {
              ...
              if (res?.success) {
                ...
                Alert.alert('Success', 'Material deleted safely.');
              }
            } catch (error: any) {
        ...
        <View style={styles.actionConsoleRow}>
          ...
          {canManage && (
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <TouchableOpacity onPress={() => handleDeleteMaterial(item._id)} style={{ paddingLeft: 16 }}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={{ textAlign: 'center', fontSize: 13, color: theme.subText, marginTop: 40 }}>
            No operational material files found matching parameters.
          </Text>
        }
      />
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  launchBtn: { flexDirection: 'row', alignItems: 'center', borderWidth: 2, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 6 },
});
